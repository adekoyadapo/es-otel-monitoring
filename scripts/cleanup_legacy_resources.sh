...
kubectl -n lab-main delete deploy main-search-load --ignore-not-found >/dev/null 2>&1 || true
...
kubectl -n lab-main delete configmap ldap-bootstrap main-search-load-script --ignore-not-found >/dev/null 2>&1 || true
...
kubectl -n lab-main delete secret ldap-admin-credentials ldap-ldaps-tls es-ldap-bind-secret es-ldap-role-mapping main-search-load-credentials --ignore-not-found >/dev/null 2>&1 || true
